...
async function processAyahs(ayahNumbers, log) {
  try {
    const tempFiles = [];
    ...
    log(`The file is saved: ${outputFile}`);
  } catch (err) {
    console.error("Error:", err);
  }
}

async function mp3create(settings, log) {
  let ayahNumbers = [];
  ...
  if (config.mode === 1) {
    ...
    log("Неверный режим в config.mode");
    return;
  }

  await processAyahs(ayahNumbers, log);
